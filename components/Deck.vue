...
<template>
  <div class="flex flex-row my-8 justify-center items-center">
    ...
    <div class="flex flex-col justify-center items-center">
      <h5 class="text-xl mb-5 font-bold text-gray-900">{{ chars.length > 0 ? 'Deck' : 'Drop cards to deck' }}</h5>
      <Draggable v-model="chars" item-key="id" group="deck" class="grid grid-cols-4 xl:grid-cols-8 gap-4 w-10/12 lg:w-10/12 bg-gray-100 p-4 rounded-lg" style="min-height: 200px; min-width: 400px;" @change="adjustDeck" @start="deckStore.setDragging(true)" @end="deckStore.setDragging(false)">
